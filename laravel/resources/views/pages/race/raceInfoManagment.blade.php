...
@section('content')
    ...
    @include('partials.herosection', ['pageTitle' => $race->race_name, 'pageSubTitle' => 'Gestion des émargements et justificatifs', 'imageUrl' => asset('img/heroSection/header.jpg')])
        ...
        <table class="admin-table w-full border-collapse">
            ...
            @foreach($members as $teamId => $teamMembers)
                ...
                <tr class="row-team bg-gray-200 font-bold border-t-2 border-gray-400">
                    ...
                    <td>
                        <form action="{{ route('manage.race.update', $race->race_id) }}" method="POST">
                            @csrf
                            @method('PUT')
    ...
    <div class="admin-table-container mb-8">
        ...
        <form action="{{ route('manage.race.importCSV', $race->race_id) }}" method="POST" enctype="multipart/form-data" class="flex items-end gap-4">
            @csrf
            <div class="flex-1">
    ...
    <script>
        document.querySelectorAll('.ajax-update').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('manage.race.update', $race->race_id) }}", {
                    method: 'POST',
                    headers: {
