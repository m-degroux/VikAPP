<header class="border-b border-gray-100 bg-white" x-data="{ open: false }">
    <nav class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center text-[15px] uppercase tracking-wider font-bold text-gray-700">
        ...
        <ul class="hidden md:flex items-center gap-6">
            ...
            <li><a href="{{ url('/raid') }}" class="hover:text-brand-green">Raids</a></li>
            ...
            @auth('admin')
                ...
                <li><a href="{{ url('/create/club') }}" class="hover:text-brand-green">Créer un club</a></li>
                <li><a href="{{ url('/manage/club') }}" class="hover:text-brand-green">Gérer les clubs</a></li>
            @endauth
            ...
            @auth('web')
                ...
                @if($user->isClubManager())
                    <li><a href="{{ route('manage.club.edit', ['club' => $user->clubId($user->user_id)]) }}" class="hover:text-brand-green border-l pl-4 border-gray-300">Club</a></li>
                    <li><a href="{{ url('/create/raid') }}" class="hover:text-brand-green border-l pl-4 border-gray-300">Créer un Raid</a></li>
                @endif


                {{-- ROLE: RAID ORGANIZER --}}
                {{-- Permission: Can manage assigned Raids and create Races within them --}}
                @if($user->isRaidOrganizer())
                    <li><a href="{{ url('/manage/raid') }}" class="hover:text-brand-green">Mes Raids</a></li>
                @endif


                {{-- ROLE: RACE MANAGER --}}
                {{-- Permission: Can manage race specifics (participants, results) --}}
                @if($user->isRaceManager())
                    <li><a href="{{ url('/manage/race') }}" class="hover:text-brand-green border-l pl-4 border-gray-300">Gérer mes courses</a></li>
                @endif
            @endauth
    ...
    <div x-show="open"
        ...
        <div class="px-6 py-4 space-y-4 text-[14px] font-bold uppercase tracking-wider">
            ...
            <a href="{{ url('/raid') }}" class="block hover:text-brand-green">Raids</a>

            {{-- OPTIONS ADMIN --}}
            @auth('admin')
                <div class="pt-2 border-t border-gray-200 space-y-3">
                    <a href="{{ url('/create/club') }}" class="block hover:text-brand-green">Créer un club</a>
                    <a href="{{ url('/manage/club') }}" class="block hover:text-brand-green">Gérer les clubs</a>
                </div>
            @endauth
            ...
            @auth('web')
                ...
                <div class="pt-2 border-t border-gray-200 space-y-3">
                    ...
                    @if($user->isClubManager())
                        <a href="{{ route('manage.club.edit', ['club' => $user->clubId($user->user_id)]) }}" class="block hover:text-brand-green">Mon Club</a>
                        <a href="{{ url('/create/raid') }}" class="block hover:text-brand-green">Créer un Raid</a>
                    @endif

                    @if($user->isRaidOrganizer())
                        <a href="{{ url('/manage/raid') }}" class="block hover:text-brand-green">Mes Raids</a>
                    @endif

                    @if($user->isRaceManager())
                        <a href="{{ url('/manage/race') }}" class="block hover:text-brand-green text-brand-green">Gérer mes courses</a>
                    @endif
