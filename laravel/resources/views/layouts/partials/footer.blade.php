    ...
    <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
        ...
        <div class="flex flex-col h-full">
            ...
            <ul class="space-y-2 text-sm text-white/90">
                ...
                <li><a href="{{ route('raid.index') }}" class="hover:text-white hover:underline decoration-2 underline-offset-4 transition">Les Raids & Courses</a></li>
            </ul>
            
            <h4 class="text-lg font-bold mt-6 mb-4 border-b border-white/20 pb-2 inline-block">Espace Membre</h4>
            <ul class="space-y-2 text-sm text-white/90">
                @if(!auth()->guard('web')->check() && !auth()->guard('admin')->check())
                    <li><a href="{{ route('login') }}" class="hover:text-white hover:underline decoration-2 underline-offset-4 transition">Connexion</a></li>
                    <li><a href="{{ route('register') }}" class="hover:text-white hover:underline decoration-2 underline-offset-4 transition">Inscription</a></li>
                @endif
                @auth('web')
                    <li><a href="{{ route('profile.edit') }}" class="hover:text-white hover:underline decoration-2 underline-offset-4 transition">Profil</a></li>
                    <li><a href="{{ route('runner.index') }}" class="hover:text-white hover:underline decoration-2 underline-offset-4 transition">Espace Coureur</a></li>
                @endauth
    ...
    <div class="border-t border-white/20 pt-6 mt-6">
        <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-white/60">
            <div class="flex flex-wrap gap-4 justify-center md:justify-start">
                ...
                <a href="{{ route('legal') }}" class="hover:text-white transition">Politique de Confidentialité (RGPD)</a>
                <a href="{{ route('legal') }}" class="hover:text-white transition">CGU</a>
            </div>
